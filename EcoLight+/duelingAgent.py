...
class DuelingAgent(NetworkAgent):
    ...
    def build_network(self):
        ...
        all_flatten_feature = tf.keras.layers.concatenate(list_all_flatten_feature, axis=1, name="all_flatten_feature")

        # shared dense layer
        shared_dense = self._shared_network_structure(all_flatten_feature, self.para_set.D_DENSE)

        # Reduce the complexity of the network by using smaller layers
        feauture_layer = Dense(64, activation='relu')(shared_dense)
        features = Dense(64, activation='relu')(feauture_layer)

        value_stream = Dense(64, activation="relu")(features)
        values = Dense(1, activation="linear")(value_stream)

        advantage_stream = Dense(64, activation="relu")(features)
        advantages = Dense(self.num_actions, activation="linear")(advantage_stream)

        qvals = values + (advantages - tf.reduce_mean(advantages, axis=1, keepdims=True))

        network = Model(inputs=[dic_input_node[feature_name] for feature_name in self.para_set.LIST_STATE_FEATURE],
                        outputs=qvals)
        network.compile(optimizer=RMSprop(lr=self.para_set.LEARNING_RATE), loss="mean_squared_error")
        network.summary()

        return network
    
    def build_memory_separate(self):
        memory_list = [[[] for j in range(self.num_actions)] for i in range(self.num_phases)]
        return memory_list
    ...
    def get_sample(self, memory_slice, dic_state_feature_arrays, Y, gamma, prefix, use_average):
        ...
        for i in range(len_memory_slice):
            ...
            f_samples.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format(
                str(pre_target), str(target), str(action), str(reward), str(next_estimated_reward)
            ))
    ...
    def train_network(self, Xs, Y, prefix, if_pretrain):
        ...
        reduce_lr = ReduceLROnPlateau(
            monitor='val_loss', factor=0.1, patience=self.para_set.REDUCE_LR_PATIENCE, min_lr=1e-6, verbose=0, mode='min')

        optimizer = RMSprop(lr=self.para_set.LEARNING_RATE)
        ...
        hist = self.q_network.fit(Xs, Y, batch_size=batch_size, epochs=epochs,
                                shuffle=False,
                                verbose=1, validation_split=0.3, callbacks=[early_stopping, reduce_lr])
    ...
    def update_network(self, if_pretrain, use_average, current_time):
        ...
        dic_state_feature_arrays = {feature_name: [] for feature_name in self.para_set.LIST_STATE_FEATURE}
        Y = []
    ...
    def _sample_memory(self, gamma, with_priority, memory, if_pretrain):
        ...
        if with_priority:
            ...
            for i in range(len_memory):
                ...
                total_reward = reward + gamma * next_estimated_reward
                target = self.q_network.predict(self.convert_state_to_input(state), verbose=0)
                pre_target = np.copy(target)
                target[0][action] = total_reward
